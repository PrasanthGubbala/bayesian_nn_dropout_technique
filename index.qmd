...
-   **Neural Network Architecture**: Develop a Bayesian Neural Network (BNN) with 4 hidden layers. The network consists of the following layers:
...
-   Our BNN model can determine with great accuracy the classification labels for determining if a loan applicant will be approved for a personal loan, but can be also be used for any type of classification task if given a correlated dataset.
...
ggsave("loss_plot.png", plot = loss_plot, width = 8, height = 6)
```
...
ggsave("accuracy_plot.png", plot = accuracy_plot, width = 8, height = 6, dpi = 300)
```
...
We were able to successfully produce a **BNN** that accurately predicts classification labels for a multi-variable dataset and keep said model from overfitting the data by utilizing the **MC Dropout** technique.
